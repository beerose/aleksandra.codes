...
believed to be deep in the grave, killed by almighty GraphQL. REST supposedly
killed SOAP. Yet, people are still using it. How's that possible?
...
The stuff that was a massive deal with RPC years ago is not necessarily relevant
anymore. Some are, but many are fine **in the context of full-stack typescript
apps**. Let's look at some of them from
...
- Having to use multi-threaded servers — JavaScript is single-threaded, but it
  has an event loop, so parallel programming is possible. Again, not a problem.
...
[A note on distributed computing](https://blog.carlosgaldino.com/a-critique-of-the-remote-procedure-call-paradigm-30-years-later.html#fn:8):
