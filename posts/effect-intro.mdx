...
## What is Effect (or how to think about it)?
...
```ts
// Attempt to recover from a missing ingredient by substituting it
...
```ts
// Ask the customer for an order, which may fail if the customer is not ready
// or if the customer is ready but the dish is missing
const askCustomer = (
...
// Retries asking the customer for an order
const getOrder = (
  customerStatus: "ready" | "not ready"
): Effect.Effect<string, CustomerNotReadyError | DishMissingError> =>
  Effect.retryOrElse(
...
```ts
// Execute the dish preparation process
const program = pipe(
...
Effect gives us a safer way to code without handling exceptions, model the
