...
export function stripMdx(md: string, options: StripMdxOptions = {}) {
  // Remove horizontal rules (stripListHeaders conflict with this rule, which is why it has been moved to the top)
  let output = md.replace(/^(-\s*?|\*\s*?|_\s*?){3,}\s*/gm, "");

  // Remove YAML frontmatter from mdx
